...
def get_slot(slot):
    ...
    return name, location, online, category, degree, deadline, notice, extended, date, vague_time, year, type, author, journal_category, ifs, journal_name, h_index, fenqu, publisher

# 讲tracker中的slot解析成后端处理的格式
# slot： {conf_name, GPE, online, conf_category, conf_degree, conf_deadline, conf_notice, conf_extended, DATE, vague_time, year, type, jounal_category, IF, journal_name, PERSON, h_index, fenqu}
# result：{name, location, category, extended, ccf, core, qualis, deadline_from, deadline_to, notice_from, notice_to, begin_from, begin_to, type, jounal_category, ifs, journal_name, author, h_index, fenqu}
def set_dict(slot) -> Dict[Text, Any]:
    name, location, online, category, degree, deadline, notice, extended, date, vague_time, year, type, author, journal_category, ifs, journal_name, h_index, fenqu, publisher = get_slot(slot)
    # 解析
    dict = {}
    ...
    if degree is not None:
        degree = getMaxLength(degree).replace(' ', '').replace('-', '').upper()
        if degree == 'B类之上' or degree == 'B类以上':
            dict['ccf'] = 'A,B'
        elif degree == 'C类之上' or degree == 'C类以上':
            dict['ccf'] = 'A,B,C'
        elif degree == 'B类之下' or degree == 'B类以下':
            dict['ccf'] = 'B,C'
        elif degree == 'A类之下' or degree == 'A类以下':
            dict['ccf'] = 'A,B,C'
        elif '类' in degree:
            dict['ccf'] = degree[0]
        elif 'CCF' in degree:
...
def key_fulls(slot) -> int:
    ...
    key3 = ['journal_category', 'ifs', 'fenqu', 'h_index', 'publisher', 'ccf', 'deadline_from']
