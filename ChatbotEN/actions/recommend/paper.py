...
class searchPaper():
    def __init__(self, dict):
        ...
        if self.slot_dic['author'] is not None and '\'s' in self.slot_dic['author']:
            self.slot_dic['author'] = self.slot_dic['author'].replace('\'s', '').strip()
    ...
    # 槽值是否为空
    def isEmpty(self):
        if self.slot_dic['author'] is None and self.slot_dic['year'] is None and self.slot_dic['conference'] is None and self.slot_dic['category'] is None:
            return True
        return False
